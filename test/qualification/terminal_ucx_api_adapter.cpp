...
terminal_ucx_api_adapter_t::terminal_ucx_api_adapter_t(
    nixlAgent &agent,
    std::size_t capacity)
    : agent_(agent) {
...
nixl_status_t
terminal_ucx_api_adapter_t::subscribeTransfer(
    nixlXferReqH *request,
    std::uint64_t owner_cookie,
    nixlTerminalEventSubscriptionH *&subscription) {
    return agent_.subscribeXferTerminal(
        channel_, request, owner_cookie, subscription);
}
