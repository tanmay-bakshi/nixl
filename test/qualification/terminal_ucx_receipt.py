...
def _validate_invocation(invocation: object) -> tuple[str, str, str]:
    ...
    _require(
        set(environment)
        == set(expected_environment) | {"NIXL_PLUGIN_DIR", "LD_LIBRARY_PATH"},
...
def _validate_population(
    ...
    :param transport: Observed UCX transport.
    :returns: Population name, callback ordering counts, handle identity, and
        generation.
...
def _validate_faults(faults: object, transport: str, engine: str) -> None:
    ...
    _require(
        notification_failure.get("fault_peer_engine") == engine
        and notification_failure.get("fault_peer_admission_receipt_held") is True,
        "notification fixture did not prove its per-engine fault boundary",
    )
...
def validate_receipt(receipt: dict[str, object]) -> None:
    ...
    for case in cases:
        ...
        _require(
            ...
            and case.get("runtime_artifacts") == runtime_artifacts,
