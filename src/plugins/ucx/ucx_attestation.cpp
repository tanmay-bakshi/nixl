...
makeDescriptorDigest(const nixl_xfer_attestation_t &attestation, std::string &digest) {
    std::string input = "nixl-descriptor-attestation-v2";
    ...
    appendUint64(input, attestation.remoteAgentHandleIdentity);
    appendUint64(input, attestation.remoteAgentGeneration);
    appendUint64(input, attestation.remoteConnectionIdentity);
    appendUint64(input, attestation.authorizedEndpointIdentities.size());
    for (uint64_t endpoint_identity : attestation.authorizedEndpointIdentities) {
...
makeEvidenceDigest(const nixl_xfer_attestation_t &attestation, std::string &digest) {
    std::string input = "nixl-transfer-evidence-v2";
    appendUint64(input, attestation.handleIdentity);
...
nixlUcxAttestationState::prepare(nixl_xfer_op_t operation,
    ...
    attestation_.remoteAgentHandleIdentity = 0;
    attestation_.remoteAgentGeneration = 0;
    attestation_.remoteConnectionIdentity = 0;
    attestation_.authorizedEndpointIdentities.clear();
    if (remote_agent != local_agent) {
        if (remote_agent_authority == nullptr ||
            remote_agent_authority->handleIdentity == 0 ||
            remote_agent_authority->generation == 0 ||
            remote_agent_authority->connectionIdentity == 0 ||
            remote_agent_authority->endpointIdentities.empty() ||
            std::any_of(remote_agent_authority->endpointIdentities.begin(),
                        remote_agent_authority->endpointIdentities.end(),
                        [](uint64_t endpoint_identity) {
                            return endpoint_identity == 0;
                        })) {
            return failLocked(
                NIXL_ERR_NOT_ALLOWED,
                "remote-agent route authority is absent or incomplete");
        }

        attestation_.remoteAgentHandleIdentity = remote_agent_authority->handleIdentity;
        attestation_.remoteAgentGeneration = remote_agent_authority->generation;
        attestation_.remoteConnectionIdentity = remote_agent_authority->connectionIdentity;
        attestation_.authorizedEndpointIdentities =
            remote_agent_authority->endpointIdentities;
        std::sort(attestation_.authorizedEndpointIdentities.begin(),
                  attestation_.authorizedEndpointIdentities.end());
        if (std::adjacent_find(attestation_.authorizedEndpointIdentities.begin(),
                               attestation_.authorizedEndpointIdentities.end()) !=
            attestation_.authorizedEndpointIdentities.end()) {
            return failLocked(
                NIXL_ERR_NOT_ALLOWED,
                "remote-agent route authority contains duplicate endpoints");
        }
    } else if (remote_agent_authority != nullptr) {
        return failLocked(
            NIXL_ERR_INVALID_PARAM,
            "loopback transfer unexpectedly supplied remote-agent authority");
    }
...
nixlUcxAttestationState::recordSegment(
    ...
    if (attestation_.remoteAgentHandleIdentity != 0 &&
        !std::binary_search(attestation_.authorizedEndpointIdentities.begin(),
                            attestation_.authorizedEndpointIdentities.end(),
                            endpoint_identity)) {
        return failLocked(
            NIXL_ERR_NOT_ALLOWED,
            "segment used an endpoint outside the remote-agent authority");
    }
...
nixlUcxAttestationState::recordFlush(size_t worker_id,
    ...
    if (attestation_.remoteAgentHandleIdentity != 0 &&
        ...
        return failLocked(
            NIXL_ERR_NOT_ALLOWED,
            "flush used an endpoint outside the remote-agent authority");
    }
...
nixlUcxAttestationState::sealCompletionLocked() {
    ...
    if (attestation_.remoteAgentHandleIdentity != 0 &&
        std::any_of(attestation_.endpoints.begin(),
                    attestation_.endpoints.end(),
                    [this](const auto &endpoint) {
                        return !std::binary_search(
                            attestation_.authorizedEndpointIdentities.begin(),
                            attestation_.authorizedEndpointIdentities.end(),
                            endpoint.endpointIdentity);
                    })) {
        return failLocked(
            NIXL_ERR_NOT_ALLOWED,
            "completion contains an endpoint outside the remote-agent authority");
    }
    attestation_.state = nixl_xfer_attestation_state_t::REMOTE_FLUSHED;
